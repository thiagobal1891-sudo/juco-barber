  ...
  return (
    <div ref={containerRef} className="relative min-h-screen bg-[#050505] overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-[100] px-6 md:px-12 py-8 flex justify-between items-center mix-blend-difference">
        ...
        >
          <span className="text-2xl font-black tracking-tighter uppercase leading-none">Vaon Studio</span>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 'success' && (
                    ...
                    >
                      ...
                      <div className="space-y-4">
                        <h2 className="text-5xl font-black uppercase tracking-tighter">¡Confirmado!</h2>
                        <div className="bg-white/5 p-6 rounded-3xl border border-white/5 space-y-3">
                          <div className="flex justify-between gap-8">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-white/20">Fecha</span>
                            <span className="text-sm font-bold uppercase">{selectedDate.format('dddd, D [de] MMMM')}</span>
                          </div>
                          <div className="flex justify-between gap-8">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-white/20">Hora</span>
                            <span className="text-sm font-bold uppercase">{selectedSlot} HS</span>
                          </div>
                          <div className="flex justify-between gap-8">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-white/20">Servicio</span>
                            <span className="text-sm font-bold uppercase">{selectedService?.name}</span>
                          </div>
                        </div>
                        <p className="text-white/40 text-sm font-medium">Nos vemos pronto en Vaon Studio.</p>
                      </div>

                      <div className="flex flex-col gap-4 w-full">
                        <a 
                          href={`https://wa.me/5491122334455?text=${encodeURIComponent(
                            `Hola! Soy ${clientInfo.name}. Acabo de reservar un turno para ${selectedService?.name} el día ${selectedDate.format('DD/MM')} a las ${selectedSlot} hs. Nos vemos!`
                          )}`}
                          target="_blank"
                          className="w-full bg-[#25D366] text-white font-black py-5 rounded-2xl hover:scale-[1.02] transition-all flex items-center justify-center gap-3 text-sm uppercase tracking-widest shadow-xl shadow-[#25D366]/20"
                        >
                        ...
                        <button 
                          onClick={resetBooking}
                          className="text-[10px] font-black uppercase tracking-[0.3em] border-b border-white pb-2 hover:opacity-40 transition w-fit mx-auto"
                        >
                          Volver al Inicio
                        </button>
                      </div>
      ...
      <footer className="py-20 px-6 md:px-12 border-t border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 opacity-20 hover:opacity-100 transition-opacity">
          <div className="text-2xl font-black tracking-tighter uppercase">Vaon Studio</div>
          <div className="flex gap-12 text-[10px] font-bold uppercase tracking-[0.2em]">
            <a href="#" className="hover:underline">Instagram</a>
            <a href="#" className="hover:underline">Facebook</a>
            <a href="#" className="hover:underline">WhatsApp</a>
          </div>
          <div className="text-[10px] font-bold uppercase tracking-[0.2em]">© 2026 Vaon Studio</div>
        </div>
      </footer>
    </div>
  );
