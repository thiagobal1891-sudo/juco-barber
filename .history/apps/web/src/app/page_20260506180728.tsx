...
export default function VaonSPA() {
  ...
  const { data: availability, isLoading: loadingSlots } = useQuery<SlotDto[]>({
    queryKey: ['availability', selectedBarber?.id, selectedDate.format('YYYY-MM-DD')],
    queryFn: () => fetcher(`/bookings/availability?barberId=${selectedBarber?.id}&date=${selectedDate.format('YYYY-MM-DD')}`),
    enabled: !!selectedBarber && step === 'date',
  });
